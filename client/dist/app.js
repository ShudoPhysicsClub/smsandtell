import { createButton, createInput, createSection, styleInputBase, toErrorText, } from './dom.js';
import { SERVER_BASE, SERVER_WS, closeWSConnection, createAccount, loginAccount, sendCallAuthOK, sendCallHangup, sendICEAnswer, sendICECandidate, sendICEOffer, setWSInboundHandler, sendSMS, } from './api.js';
...
let statusNode = null;
let inboxNode = null;
let signalInboxNode = null;
let toastHostNode = null;
let messageFeedNode = null;
let threadListNode = null;
let selfNumberBadgeNode = null;
let setActiveScreenRef = null;
let syncAuthUIRef = null;
let syncMessageFeedRef = null;
let activeScreenKey = 'login';
let callPeer = null;
...
let callPhase = 'idle';
let syncCallUIRef = null;
let syncCallRuntimeRef = null;
let callStartedAt = 0;
let callTimerId = null;
let localMicMuted = false;
let micAudioCtx = null;
let micMeterRafId = null;
let updateMicLevelRef = null;
let lastToastKey = '';
let lastToastAt = 0;
...
async function loadThread(owner) {
    if (!owner)
        return null;
...
async function saveThread(owner, items, contacts) {
    if (!owner)
        return;
    const db = await openChatDB();
    const record = {
...
function showErrorToast(message) {
    ...
    if (lastToastKey === message && now - lastToastAt < 1800) {
        lastToastCount += 1;
        const latest = toastHostNode.lastElementChild;
        if (latest) {
...
function setCallPhase(phase, note) {
...
function openNodeWS(number, jwt) {
    return new Promise((resolve, reject) => {
        ...
        ws.onmessage = (event) => {
            ...
            if (data.action === 'ice_offer' || data.action === 'ice_answer' || data.action === 'ice_candidate') {
                ...
                handleSignalAction(String(data.action), data.data).catch((err) => setErrorStatus(err));
                return;
            }
...
function waitAuthenticated(timeoutMs = 6000) {
    return new Promise((resolve, reject) => {
        if (isAuthenticated) {
            resolve();
            return;
...
async function ensurePeerForTarget(target) {
    if (!target)
        throw new Error('missing call target');
    if (callPeer && activeCallPeer === target)
        return callPeer;
    cleanupCall();
...
async function handleSignalAction(action, payload) {
    const body = (payload ?? {});
    const from = String(body.from ?? '').trim();
    const to = String(body.to ?? '').trim();
    if (to && to !== currentNumber)
        return;
    if (action === 'ice_offer') {
...
export function buildUI() {
    ...
    let disconnectXButton = null;
    const syncAuthUI = () => {
        h1.style.display = isAuthenticated ? 'none' : 'block';
        if (statusNode)
            statusNode.style.display = isAuthenticated ? 'none' : 'block';
    ...
    const setActiveScreen = (key) => {
        ...
        const targetKey = isAuthenticated && key !== 'chat'
            ? 'chat'
            : key;
        activeScreenKey = targetKey;
        const keys = ['login', 'signup', 'chat'];
    ...
    const makeCard = (title) => {
        ...
        card.__outerWrap = outerWrap;
        return card;
    };
    const makeFormGroup = (label, input) => {
    ...
    const makeLinkBtn = (label, onClick) => {
    ...
    loginScreen.appendChild(loginCard.__outerWrap ?? loginCard);
    // --- 新規登録画面 ---
    ...
    signupScreen.appendChild(signupCard.__outerWrap ?? signupCard);
    ...
    let mobileChatPane = 'threads';
    let mobileBackButton = null;
    let peerNameField = null;
    let mobileOpenConversationButton = null;
    ...
    const syncChatLayout = () => {
        ...
        syncPaneVisibility();
        if (mobileBackButton)
            mobileBackButton.style.display = isMobileChatLayout ? '' : 'none';
        if (peerNameField)
            peerNameField.style.display = isMobileChatLayout ? 'none' : '';
        if (mobileOpenConversationButton)
            mobileOpenConversationButton.style.display = isMobileChatLayout ? '' : 'none';
    };
    ...
    const threadPeer = (msg) => (msg.direction === 'out' ? msg.to : msg.from);
    const startComposeThread = () => {
    ...
    const parseInboundMessage = (raw) => {
        ...
        const direction = from && from === currentNumber ? 'out' : 'in';
    ...
    const renderChatItems = () => {
        ...
        for (const msg of visible.slice(-80)) {
            ...
            const stateLabel = msg.status === 'failed'
                ? `失敗 ${msg.reason ?? ''}`.trim()
                : msg.status === 'sending'
                    ? '送信中'
                    : msg.status === 'received'
    ...
    syncMessageFeedRef = (messages) => {
        ...
        for (const raw of messages) {
            ...
            if (!parsed)
                continue;
            if (chatItems.some((x) => x.id === parsed.id))
                continue;
    ...
    setWSInboundHandler((data) => {
        ...
        if (normalizedAction === 'ice_offer' ||
            ...
            handleSignalAction(normalizedAction, data.data).catch((err) => setErrorStatus(err));
            return;
        }
        if (data.error) {
            setErrorStatus(`node error: ${String(data.error)}`);
    ...
    const screenKeys = ['login', 'signup', 'chat'];
    ...
    const doLogin = async (opts) => {
        if (isLoggingIn)
            throw new Error('ログイン処理中です。完了をお待ちください');
        isLoggingIn = true;
        try {
            ...
            if (!opts?.silent)
                setStatus(`ログイン成功: ${currentNumber}`);
            // チャット画面に遷移
    ...
    const startCallFlow = async () => {
        try {
            ...
            const from = currentNumber;
            if (!to)
                throw new Error('call to is required');
            if (!from)
                throw new Error('from is required');
